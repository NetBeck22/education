...
# ══════════════════════════════════════════════════════════════════════════════
with tab1:
    st.subheader("Language integration needs in Flanders, 2014-2024")
    st.markdown(
        "This section keeps the language-integration story to three visuals. The score combines "
        "three signals that matter for local policy: recent newcomer intake, the share of residents "
        "with non-EU nationality, and the share of pupils whose home language is not Dutch. It should "
        "be read as a planning signal: where will municipalities, schools and integration services "
        "need the most coordination?"
    )
    ...
    lang_all["language_need_score"] = (
        minmax(lang_all["pct_geen_nl_avg"]).fillna(0) * 0.45
        + minmax(lang_all["nieuwkomers_per_1000"].fillna(0)) * 0.30
        + minmax(lang_all["pct_non_eu27"].fillna(0)) * 0.25
    ).round(1)

    lang_2014 = lang_all[lang_all["year"] == 2014][
        ["CD_REFNIS", "language_need_score", "pct_geen_nl_avg", "nieuwkomers_per_1000"]
    ].rename(columns={
        "language_need_score": "score_2014",
        "pct_geen_nl_avg": "home_language_2014",
        "nieuwkomers_per_1000": "newcomers_2014",
    })
    lang_2024 = lang_all[lang_all["year"] == 2024].merge(lang_2014, on="CD_REFNIS", how="left")
    lang_2024["score_change_2014_2024"] = (lang_2024["language_need_score"] - lang_2024["score_2014"]).round(1)
    lang_2024["home_language_change_2014_2024"] = (lang_2024["pct_geen_nl_avg"] - lang_2024["home_language_2014"]).round(1)
    lang_2024["newcomer_change_2014_2024"] = (lang_2024["nieuwkomers_per_1000"] - lang_2024["newcomers_2014"]).round(1)

    lang_province_year = (
        lang_all.groupby(["year", "TX_PROV_DESCR_NL"], as_index=False)
        .agg(
            language_need_score=("language_need_score", "mean"),
            pct_geen_nl_avg=("pct_geen_nl_avg", "mean"),
            nieuwkomers_per_1000=("nieuwkomers_per_1000", "mean"),
            pct_non_eu27=("pct_non_eu27", "mean"),
        )
    )
    lang_province_2014 = lang_province_year[lang_province_year["year"] == 2014][
        ["TX_PROV_DESCR_NL", "language_need_score"]
    ].rename(columns={"language_need_score": "score_2014"})
    lang_province_2024 = lang_province_year[lang_province_year["year"] == 2024].merge(
        lang_province_2014, on="TX_PROV_DESCR_NL", how="left"
    )
    lang_province_2024["score_change"] = (
        lang_province_2024["language_need_score"] - lang_province_2024["score_2014"]
    ).round(1)

    top_lang_province = lang_province_2024.sort_values("language_need_score", ascending=False).iloc[0]
    fastest_lang_province = lang_province_2024.sort_values("score_change", ascending=False).iloc[0]
    top_lang_muni = lang_2024.sort_values("language_need_score", ascending=False).iloc[0]
    fastest_lang_muni = lang_2024.sort_values("score_change_2014_2024", ascending=False).iloc[0]

    k1, k2, k3 = st.columns(3)
    k1.metric("Highest province in 2024", top_lang_province["TX_PROV_DESCR_NL"], f"{top_lang_province['language_need_score']:.1f}/100")
    k2.metric("Fastest rising province", fastest_lang_province["TX_PROV_DESCR_NL"], f"+{fastest_lang_province['score_change']:.1f}")
    k3.metric("Highest municipality in 2024", top_lang_muni["TX_DESCR_NL"], f"{top_lang_muni['language_need_score']:.1f}/100")

    st.markdown(
        f"In 2024, **{top_lang_province['TX_PROV_DESCR_NL']}** has the highest average language-integration "
        f"need. The strongest increase since 2014 is in **{fastest_lang_province['TX_PROV_DESCR_NL']}**. "
        f"At municipality level, **{top_lang_muni['TX_DESCR_NL']}** is the highest-need case, while "
        f"**{fastest_lang_muni['TX_DESCR_NL']}** has risen fastest over the full period. For policy makers, "
        "that separates two questions: where do we need stable long-term capacity, and where do we need "
        "early action because demand is growing quickly?"
    )

    st.divider()
    st.subheader("1. Province picture: where are language needs highest?")
    st.markdown(
        "This chart starts at province level because it is the clearest way to brief a policy audience. "
        "Use the play button to move from 2014 to 2024 and see whether needs are stable, rising, or shifting "
        "between provinces."
    )
    fig_lang_province = px.bar(
        lang_province_year.sort_values(["year", "language_need_score"]),
        x="language_need_score",
        y="TX_PROV_DESCR_NL",
        orientation="h",
        color="language_need_score",
        animation_frame="year",
        range_x=[0, max(100, float(lang_province_year["language_need_score"].max()) * 1.05)],
        color_continuous_scale="YlOrRd",
        text="language_need_score",
        labels={
            "language_need_score": "Language need score",
            "TX_PROV_DESCR_NL": "Province",
            "year": "Year",
        },
    )
    fig_lang_province.update_traces(texttemplate="%{text:.1f}", textposition="outside")
    fig_lang_province.update_layout(
        height=430,
    ...
    st.plotly_chart(fig_lang_province, use_container_width=True)

    st.markdown(
        "A high province score points to structural capacity needs: enough Dutch-language learning, "
        "interpretation support, multilingual communication and coordination between municipalities, "
        "schools and integration organisations. A fast-rising province is an early-warning signal."
    )

    st.divider()
    st.subheader("2. Municipality map: where should local coordination focus?")
    st.markdown(
        "The map shows the same score at municipality level. This is where the play button is especially "
        "useful: it reveals whether needs remain concentrated in a few urban centres or spread into "
        "smaller municipalities that may have less existing support capacity."
    )
    fig_lang_map = add_play_choropleth(
        lang_all,
        "language_need_score",
        "Language need score",
        color_scale="YlOrRd",
        range_max=max(70, float(lang_all["language_need_score"].quantile(0.98))),
        height=590,
    )
    st.plotly_chart(fig_lang_map, use_container_width=True)

    st.markdown(
        f"By 2024, **{top_lang_muni['TX_DESCR_NL']}** has the highest local score. Municipalities that stay "
        "dark over time are candidates for sustained language-service capacity. Municipalities that become "
        "darker after 2014 are candidates for early coordination before pressure becomes harder to manage."
    )

    st.divider()
    st.subheader("3. Local support planning: which municipalities need capacity first?")
    st.markdown(
        "This final chart turns the map into a policy conversation. Choose a province and compare the "
        "municipalities with the highest 2024 language need across the full 2014-2024 period. The title is "
        "intentionally practical: these are not 'problem municipalities', but places where support should be "
        "planned and discussed first."
    )
    lang_province_options = lang_province_2024.sort_values("language_need_score", ascending=False)["TX_PROV_DESCR_NL"].tolist()
    selected_lang_province = st.selectbox(
        "Province",
        lang_province_options,
        key="t1_priority_province",
    )
    lang_province_munis = (
        lang_2024[lang_2024["TX_PROV_DESCR_NL"] == selected_lang_province]
        .sort_values("language_need_score", ascending=False)
    )
    default_lang_munis = lang_province_munis.head(8)["TX_DESCR_NL"].tolist()
    selected_lang_munis = st.multiselect(
        "Municipalities to compare",
        lang_province_munis["TX_DESCR_NL"].tolist(),
        default=default_lang_munis,
        key="t1_priority_munis",
    )
    if selected_lang_munis:
        lang_trend = lang_all[lang_all["TX_DESCR_NL"].isin(selected_lang_munis)].copy()
        fig_lang_trend = px.line(
            lang_trend,
            x="year",
            y="language_need_score",
            color="TX_DESCR_NL",
            markers=True,
            labels={
                "year": "Year",
        ...
        selected_lang_summary = lang_2024[lang_2024["TX_DESCR_NL"].isin(selected_lang_munis)].sort_values(
            "score_change_2014_2024", ascending=False
        )
        fastest_selected_lang = selected_lang_summary.iloc[0]
        highest_selected_lang = selected_lang_summary.sort_values("language_need_score", ascending=False).iloc[0]
        st.markdown(
            f"For **{selected_lang_province}**, **{highest_selected_lang['TX_DESCR_NL']}** has the highest "
            f"2024 language need among the selected municipalities. **{fastest_selected_lang['TX_DESCR_NL']}** "
            "rose the most between 2014 and 2024. That gives policy makers two useful lists: places that need "
            "sustained capacity now, and places where demand is growing quickly enough to justify earlier action."
        )
    else:
...
with tab2:
    ...
    st.markdown(
        "Education pressure is a planning score for municipalities, not a measure of school quality. "
        "It combines three education-relevant demand signals: pupils whose home language is not Dutch "
        "in primary and secondary education, youth with non-Belgian nationality, and newcomer intensity. "
        "The score helps identify where schools and local services may need extra language support, "
        "parent communication, transition guidance and coordination capacity."
    )
    st.info(
        "Score definition: 45% normalised non-Dutch home-language share, 35% normalised youth "
        "non-Belgian share, and 20% normalised newcomers per 1,000 residents. The priority rank "
        "combines the 2024 score level with growth since 2014."
    )
    ...
    edu_all["newcomer_component"] = (minmax(edu_all["nieuwkomers_per_1000"].fillna(0)) * 0.20).round(1)
    edu_all["school_pressure_score"] = (
        edu_all["home_language_component"]
        + edu_all["youth_origin_component"]
        + edu_all["newcomer_component"]
    ).round(1)
    ...
    edu_2024["level_rank"] = edu_2024["school_pressure_score"].rank(method="min", ascending=False)
    edu_2024["growth_rank"] = edu_2024["score_change_2014_2024"].rank(method="min", ascending=False)
    ...
    st.divider()
    st.subheader("2. Municipality priority ranking: what drives the pressure?")
    st.markdown(
        "Use the year filter to inspect the highest-ranking municipalities and the components behind "
        "their score. The stacked bars show how much each dimension contributes after normalisation: "
        "home language carries 45% of the score, youth nationality 35%, and newcomer intensity 20%."
    )
    selected_edu_year = st.select_slider("Year", options=YEARS, value=2024, key="t2_priority_year")
    ...
    edu_year_top20 = edu_year.nlargest(20, "school_pressure_score").sort_values("school_pressure_score", ascending=True)

    fig_components = go.Figure()
    component_labels = {
        "home_language_component": "Home language",
        "youth_origin_component": "Youth nationality",
        "newcomer_component": "Newcomer intensity",
    }
    component_colors = {
        "home_language_component": "#B23A48",
        "youth_origin_component": "#2E86AB",
        "newcomer_component": "#E09F3E",
    }
    for component, label in component_labels.items():
        fig_components.add_bar(
            y=edu_year_top20["TX_DESCR_NL"],
            x=edu_year_top20[component],
            name=label,
            orientation="h",
    ...
    ranking_table = edu_year.sort_values("priority_rank")[ranking_cols].rename(columns={
    ...
    st.dataframe(ranking_table.head(30), use_container_width=True, hide_index=True)

    st.divider()
    st.subheader("3. Local support planning: which municipalities need capacity first?")
    st.markdown(
        "Choose a province, then either show all municipalities in that province or select specific "
        "municipalities to compare. This turns the ranking into a concrete action list: which places need "
        "sustained support, and which places are rising fast?"
    )
    ...
    selection_mode = st.radio(
        ...
        horizontal=True,
        key="t2_muni_selection_mode",
    )
    if selection_mode == "All municipalities in province":
        selected_munis = province_2024_munis["TX_DESCR_NL"].tolist()
    else:
    ...
    if selected_munis:
        ...
        if selection_mode == "All municipalities in province":
            fig_muni_trend.update_layout(showlegend=False)
