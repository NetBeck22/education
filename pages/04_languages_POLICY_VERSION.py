...
This dashboard brings together three connected dimensions of demographic change in Flanders:
language support needs, education pressure, and labour-market integration.

The goal is to help local and regional policy makers identify where demographic change is
creating pressure on schools, public services and labour-market participation. Rather than
looking at migration only through population counts, the dashboard combines indicators on
home language, newcomer inflow, youth diversity and employment outcomes to show where
integration capacity may need to increase.

The three sections should be read as a connected policy story:

- **Language Support Needs** identifies municipalities where schools and local services face increasing language-support demand.
- **Education Pressure** shows where demographic change is likely to create additional pressure on educational capacity and youth support systems.
- **Labour Market & Migration** connects these demographic trends to employment outcomes and labour-market integration.

Together, the dashboard supports evidence-based decisions on:
school capacity planning, integration policy, local support services, labour-market activation
...
try:
    ...
    st.write("Loaded labour-market datasets successfully.")

except Exception as e:
    st.error(f"Error while loading labour-market datasets: {e}")
    st.stop()
...
required_origin_cols = ["belgian_bg", "eu27", "non_eu27", "total"]

missing_cols = [c for c in required_origin_cols if c not in origin_vl.columns]

if missing_cols:
    st.error(f"Missing columns in origin dataset: {missing_cols}")
    st.write("Available columns:", origin_vl.columns.tolist())
    st.stop()
...
js_2024["wzg_pct"] = js_2024["wzg_total"].round(2)
...
st.markdown("""
The three sections of this dashboard describe different stages of the same long-term policy challenge.

Municipalities with rapidly rising newcomer inflow and non-Dutch home-language shares are
often the same municipalities where education pressure increases first. Over time, these
demographic shifts also become visible in labour-market outcomes, particularly in the gap
between Belgian-origin and non-EU-origin job-seeker rates.

This means that language support, education policy and labour-market integration should not
be treated as separate domains. Early investment in language acquisition, school support and
local integration capacity can influence later employment outcomes and reduce long-term
social pressure.

The dashboard therefore highlights where coordinated policy responses may be most urgent:
municipalities combining rapid demographic change, rising education pressure and weaker
labour-market integration outcomes.

For policy makers, the central message is that demographic change is not only a migration
question, but also a capacity-planning question across education, local services and the labour market.
""")
